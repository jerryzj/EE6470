...
class SimpleBus : public sc_core::sc_module, public MemoryMap{
	...
	public:
		...
		SimpleBus(sc_core::sc_module_name name, double clock_period_in_ps = 1000, bool trace = false, bool masked = true ) :
			sc_core::sc_module(name), 
			MemoryMap( name, NR_OF_INITIATOR_SOCKETS), 
		...
		void initiatorBTransport(int SocketId, transaction_type& trans, sc_core::sc_time& t) {
			...
			if ( portId < 0 ) {
				std::cout << "ERROR: " << name()
					<< ": initiatorBTransport()"
					<< ": Invalid (undefine memory mapped) address == " << std::endl;
				assert(false); 
			}
