...
{
public:
  MemoryMap( char const * the_memory_map_name, const unsigned int the_no_of_targets )
...
public:
  void setDecode(int portId, Addr lo, Addr hi) {
    ...
    if (lo > hi) {
      printf(
          "Error configuring TLM decoder %s: lo (0x%llX) cannot be greater than hi (0x%llX)\n",
          memory_map_name().c_str(),
          lo,
          hi
          );
